...
if ($text === "🍄 Expeditions" && $chatType === "private") {
	...
	if ($rowUsersUtilities = $selectUsersUtilities->fetch(PDO::FETCH_ASSOC)) {
		try {
			...
			if ($numExpeditions > 0) {
				sendMessage($chatId, "🍄 [ <code>EXPEDITIONS</code> ] (Level <b>" . $rowUsersExpeditions["level"] . "</b>)\nEmbark on thrilling journeys across unknown lands! Each Expedition promises rewards and hidden dangers—face them for a chance at rare loot and legendary encounters!\n\n" . $string . "ℹ️ <i>To advance to the next level, you need " . number_format($experienceRequired) . " (Experience), <a href='https://t.me/this-is-an-example-link'>here</a> is a complete list of the items you can obtain.</i>", true, false, false, '&reply_markup={"inline_keyboard":[[{"text":"🔍 (search)","callback_data":"expeditions_search"}]],"resize_keyboard":true}');
			} else {
				sendMessage($chatId, "🍄 [ <code>EXPEDITIONS</code> ] (Level <b>" . $rowUsersExpeditions["level"] . "</b>)\nEmbark on thrilling journeys across unknown lands! Each Expedition promises rewards and hidden dangers—face them for a chance at rare loot and legendary encounters!\n\nℹ️ <i>To advance to the next level, you need " . number_format($experienceRequired) . " (Experience), <a href='https://t.me/this-is-an-example-link'>here</a> is a complete list of the items you can obtain.</i>", true, false, false, '&reply_markup={"inline_keyboard":[[{"text":"🔍 (search)","callback_data":"expeditions_search"}]],"resize_keyboard":true}');
			}
		} catch (Exception $exception) {
			systemLogs($pdo, $userId, "ERROR", $exception->getMessage(), $text);
			exit;
