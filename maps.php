	...
	try {
		...
		if (!empty($lootItems)) {
			for ($i = 0; $i < $maxDrops; $i++) {
				...
				$dropRate = $randomLoot['drop_rate'] / 100;
				if (mt_rand() / mt_getrandmax() <= $dropRate) {
					...
					if ($itemDetails) {
						if ($randomLoot['item_id'] != 22) {
							...
							$droppedItemIds[] = $randomLoot['item_id'];
		...
		if (!empty($droppedItems)) {
			$lootString = ", " . implode(", ", array_slice($droppedItems, 0, -1));
			if (count($droppedItems) > 1) {
				$lootString .= " and " . end($droppedItems);
			} else {
				$lootString = ", " . $droppedItems[0];
			}
		} else {
			$lootString = "";
		}
