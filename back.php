...
if ($text === "🔙 Go Back" && $chatType === "private") {
	...
	if ($rowUsersUtilities = $selectUsersUtilities->fetch(PDO::FETCH_ASSOC)) {
		try {
			...
			sendMessage($chatId, "💳 [ <code>SHOP</code> ]\nAll purchases are subject to our <b>Terms of Service</b>. By proceeding you confirm that you have read these <a href='https://t.me/this-is-an-example-link'>Terms of Service</a>. Please review them before making a purchase.", true, false, false, '&reply_markup={"keyboard":[["😈 Darkness Pass", "💎 Diamonds"],["🎮 Games"],["🔙 Go Back"]],"resize_keyboard":true}');
		} catch (Exception $exception) {
			systemLogs($pdo, $userId, "ERROR", $exception->getMessage(), $text);
			exit;
		}
...
if ($text === "🔙 Go Back" && $chatType === "private") {
	...
	if ($rowUsersUtilities = $selectUsersUtilities->fetch(PDO::FETCH_ASSOC)) {
		try {
			...
			sendMessage($chatId, "⚗️ [ <code>ALCHEMY</code> ] (Level <b>" . $rowUsersAlchemy["level"] . "</b>)\nThe ancient art of transforming materials into powerful potions and rare artifacts, harnessing the remnants of the shattered suns.\n\nℹ️ <i>To advance to the next level, you need " . number_format($experienceRequired) . " (Experience), <a href='https://t.me/this-is-an-example-link'>here</a> is a complete list of the items that can be synthesize.</i>", true, false, false, '&reply_markup={"keyboard":[["🧪 Potions"],["🔙 Go Back"]],"resize_keyboard":true}');
		} catch (Exception $exception) {
			systemLogs($pdo, $userId, "ERROR", $exception->getMessage(), $text);
			exit;
		}
...
if ($text === "🔙 Go Back" && $chatType === "private") {
	...
	if ($rowUsersUtilities = $selectUsersUtilities->fetch(PDO::FETCH_ASSOC)) {
		try {
			...
			sendMessage($chatId, "⚒️ [ <code>BLACKSMITH</code> ] (Level <b>" . $rowUsersBlacksmith["level"] . "</b>)\nMaster the art of forging legendary weapons and armor. Utilize mystical materials to create powerful artifacts.\n\nℹ️ <i>To advance to the next level, you need " . number_format($experienceRequired) . " (Experience). <a href='https://t.me/this-is-an-example-link'>Here</a> is a complete list of items you can forge.</i>", true, false, false, '&reply_markup={"keyboard":[["⚙️ Forge"],["🔙 Go Back"]],"resize_keyboard":true}');
		} catch (Exception $exception) {
			systemLogs($pdo, $userId, "ERROR", $exception->getMessage(), $text);
			exit;
		}
